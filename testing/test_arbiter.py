...
def create_violating_command(policy: arbiter.models.Policy) -> str:
    command = "stress-ng"
    cpu = policy.query_params.get("cpu_threshold", None)
    if cpu:
        command += f" --cpu {math.ceil(cpu)}"
    mem = policy.query_params.get("memory_threshold", None)
    if mem:
        command += f" --vm 1 --vm-bytes {math.ceil(mem)}g --vm-populate --vm-keep --vm-madvise willneed"
    time = policy.query_params.get("time_window", None)
    if not time:
        time = 10
    command += f" --timeout {time}"
    return command
...
def create_violation(target, policy):
    ...
    window = int(policy.timewindow.total_seconds())
...
def duration(policy):
    return int(policy.penalty.duration.total_seconds()) + 1
...
def test_single_target_single_policy(
    short_low_harsh_policy, target1
):
    #start bad behavior
    ...
    print(Target.objects.all(), target1.host)
    assert db_target1 != None #here

    #make sure correct limits were applied
    should_be = list(short_low_harsh_policy.penalty.limits.all())
    assert list(db_target1.last_applied.all()) == should_be
    ...
    assert len(db_target1.last_applied.all()) == 0
...
def test_multiple_target_single_policy(
    ...
    should_be = list(short_low_harsh_policy.penalty.limits.all())
    assert list(db_target1.last_applied.all()) == should_be
    assert list(db_target2.last_applied.all()) == should_be
    ...
    assert db_target2 != None
    assert len(db_target1.last_applied.all()) == 0
    assert len(db_target2.last_applied.all()) == 0
...
def test_single_target_distinct_policy(
    ...
    db_target1 = Target.objects.filter(unit=target1.unit, host=target1.host).first()
    assert db_target1 != None
    should_be = list(short_mid_soft_policy.penalty.limits.all())
    assert list(db_target1.last_applied.all()) == should_be
...
#############################################################
@pytest.mark.django_db(transaction=True)
def test_reviolation_while_in_violation_single_policy(
    short_low_unset_policy, target1
):
    """
    ...
    target_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=short_low_unset_policy)
    ...
    assert not target_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()

    # cleanup and make sure process ends
    time.sleep(runtime/2)

@pytest.mark.django_db(transaction=True)
def test_violation_in_grace_single_policy(
    grace_no_lookback_policy, target1
):
    """
    ...
    target_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=grace_no_lookback_policy)
    first_violation = target_policy_violations.first()
    assert first_violation != None

    # now continue bad behavior and see if new violation was made
    runtime = create_violation(target1, grace_no_lookback_policy)
    time.sleep(grace_no_lookback_policy.penalty.duration.total_seconds())
    arbiter.eval.evaluate([grace_no_lookback_policy])
...
@pytest.mark.django_db(transaction=True)
def test_violation_after_grace_single_policy(
    grace_no_lookback_policy, target1
):
    """
    ...
    assert target_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()
...
def test_violation_in_grace_multiple_policy(
    ...
    target_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host)
    target_grace_policy_violations = target_violations.filter(policy=grace_no_lookback_policy)
    target_soft_policy_violations  = target_violations.filter(policy=short_low_soft_policy)
    ...
    assert not target_grace_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()
    assert target_soft_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()

    # cleanup and make sure process ends
    time.sleep(max(runtime - (grace_no_lookback_policy.grace_period.total_seconds()-2), 1))
...
def test_violation_in_grace_single_policy_multiple_targets(
    ...
    #eval and make sure first violation applied then grab it
    arbiter.eval.evaluate([grace_no_lookback_policy])
    target1_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=grace_no_lookback_policy)
    ...
    time.sleep(grace_no_lookback_policy.penalty.duration.total_seconds() + 2)
    arbiter.eval.evaluate([grace_no_lookback_policy])

    # make sure no new violations were added for target the original target, but one was for the second target
    target2_policy_violations = Violation.objects.filter(target__unit = target2.unit, target__host = target2.host, policy=grace_no_lookback_policy)
    assert not target1_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()
    assert target2_policy_violations.filter(timestamp__gt = first_violation.timestamp).exists()
...
#############################################################
@pytest.mark.django_db(transaction=True)
def test_repeat_violation_scales(
    long_lookback_no_grace_policy, target1
):
    """
    ...
    target_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=long_lookback_no_grace_policy)
    first_violation = target_policy_violations.first()
    assert first_violation != None

    # now continue bad behavior and see if new violation was made
    runtime = create_violation(target1, long_lookback_no_grace_policy)
    time.sleep(long_lookback_no_grace_policy.penalty.duration.total_seconds())
    arbiter.eval.evaluate([long_lookback_no_grace_policy])
    ...
    time.sleep(max(runtime - long_lookback_no_grace_policy.penalty.duration.total_seconds(), 1))
...
def test_repeat_violation_scales_multiple_targets(
    ...
    target1_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=long_lookback_no_grace_policy)
    target2_policy_violations = Violation.objects.filter(target__unit = target2.unit, target__host = target2.host, policy=long_lookback_no_grace_policy)
    ...
    time.sleep(long_lookback_no_grace_policy.penalty.duration.total_seconds()+1)
    arbiter.eval.evaluate([long_lookback_no_grace_policy])

    # make sure a new violation was added for target with that policy
    second_violation_target1 = target1_policy_violations.filter(timestamp__gt = first_violation_target1.timestamp).first()
    first_violation_target2 = target2_policy_violations.filter(timestamp__gt = first_violation_target1.timestamp).first()
...
def test_repeat_violation_scales_multiple_policy(
    ...
    target_long_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=long_lookback_no_grace_policy)
    target_short_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=short_lookback_no_grace_policy)
    ...
    runtime = create_violation(target1, long_lookback_no_grace_policy)
    time.sleep(long_lookback_no_grace_policy.penalty.duration.total_seconds())
    arbiter.eval.evaluate([long_lookback_no_grace_policy, short_lookback_no_grace_policy])

    # make sure a new violation was added for target with that policy
    second_violation = target_long_policy_violations.filter(timestamp__gt = first_violation.timestamp).first()
    first_short_violation = target_short_policy_violations.filter(timestamp__gt = first_violation.timestamp).first()
    assert second_violation != None
...
@pytest.mark.django_db(transaction=True)
def test_short_lookback_forgets_old_violations(
    short_lookback_no_grace_policy, target1
):
    """
    ...
    target_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=short_lookback_no_grace_policy)
    first_violation = target_policy_violations.first()
    assert first_violation != None

    # now continue bad behavior and see if new violation was made
    runtime = create_violation(target1, short_lookback_no_grace_policy)
    time.sleep(short_lookback_no_grace_policy.penalty.duration.total_seconds()+1)
    arbiter.eval.evaluate([short_lookback_no_grace_policy])
    ...
    # make sure a new violation was added for target with that policy
    third_violation = target_policy_violations.filter(timestamp__gt = second_violation.timestamp).first()
...
@pytest.mark.django_db(transaction=True)
def test_lookback_and_grace(
    long_lookback_with_grace_policy, target1
):
    """
    ...
    target_policy_violations = Violation.objects.filter(target__unit = target1.unit, target__host = target1.host, policy=long_lookback_with_grace_policy)
    first_violation = target_policy_violations.first()
    assert first_violation != None

    # now continue bad behavior and see if new violation was made
    runtime = create_violation(target1, long_lookback_with_grace_policy)
    time.sleep(long_lookback_with_grace_policy.penalty.duration.total_seconds())
    arbiter.eval.evaluate([long_lookback_with_grace_policy])
