...
def harsh_penalty(db, harsh_limit_cpu, harsh_limit_mem):
    penalty = Penalty.objects.create(
        name="harsh_penalty", duration=timedelta(seconds=5), repeat_offense_scale = 0
    )
...
def soft_penalty(db, soft_limit_cpu, soft_limit_mem):
    penalty = Penalty.objects.create(
        name="harsh_penalty", duration=timedelta(seconds=10), repeat_offense_scale = 0
    )
...
def unset_penalty(db, unset_limit_cpu, unset_limit_mem):
    penalty = Penalty.objects.create(
        name="unset_penalty", duration=timedelta(seconds=10), repeat_offense_scale = 1
    )
