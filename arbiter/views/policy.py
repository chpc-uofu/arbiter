...
class UsagePolicyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            "description",
            "penalty_duration",
            "repeated_offense_scalar",
            "grace_period",
            "repeated_offense_lookback",
            "lookback",
        ]
        widgets = {
            "grace_period": forms.TimeInput(),
            "repeated_offense_lookback": forms.TimeInput(),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if query := self.instance.query:
            self.fields["cpu_threshold"].initial = nsec_to_cores(
                query["params"]["cpu_threshold"]
            )
            self.fields["mem_threshold"].initial = bytes_to_gib(
                query["params"]["mem_threshold"]
            )
    ...
    def save(self, commit=True):
        ...
        policy.query = QueryData.build_query(
            lookback=policy.lookback, domain=policy.domain, params=params
        ).json()
...
def new_usage_policy(request):
    ...
    return render(
        request, "arbiter/change_view.html", {"form": form, "navbar": navbar(request)}
    )
...
class BasePolicyForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        policy.query = QueryData.raw_query(
            f'systemd_unit_cpu_usage_ns{{instance=~"{policy.domain}"}}'
        ).json()
        policy.save()
